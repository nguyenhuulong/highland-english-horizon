...
export default function StoriesPage() {
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
          <button onClick={() => setView("list")} style={{ padding: "8px 16px", borderRadius: 10,
            border: "1.5px solid var(--border)", background: "var(--surface)", cursor: "pointer",
            fontWeight: 700, fontFamily: "var(--font-body)" }}>← Danh sách</button>
  ...
  if (view === "read" && readingStory) {
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
          <button onClick={() => { setView("list"); setReadingStory(null); }}
            style={{ padding: "8px 16px", borderRadius: 10, border: "1.5px solid var(--border)",
              background: "var(--surface)", cursor: "pointer", fontWeight: 700, fontFamily: "var(--font-body)" }}>
            ← Danh sách
          </button>
        </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.6rem", margin: 0 }}>
            📖 Truyện tranh
          </h1>
          <p style={{ color: "var(--text-muted)", fontSize: "0.88rem", marginTop: 4 }}>
            {stories.length} truyện — chọn nhân vật + bối cảnh + chủ đề để AI tự tạo
          </p>
        </div>
        <button onClick={() => setView("create")}
          style={{ padding: "11px 22px", borderRadius: 12, background: "var(--primary)", color: "#fff",
            border: "none", cursor: "pointer", fontWeight: 800, fontFamily: "var(--font-body)", fontSize: "0.95rem" }}>
          + Tạo truyện mới
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
        {stories.map((s) => {
          ...
          return (
            <div key={s.id} style={{ background: "var(--bg-card)", borderRadius: 16,
              border: "1.5px solid var(--border)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
              {/* Cover */}
              <div style={{ height: 180, background: "linear-gradient(135deg,#ffecd2,#fcb69f)",
                position: "relative", overflow: "hidden" }}>
                {firstPanel?.generatedImageUrl ? (
                  <img src={firstPanel.generatedImageUrl} alt={s.title}
                    style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <div style={{ height: "100%", display: "flex", alignItems: "center",
                    justifyContent: "center", fontSize: "4rem" }}>{eg?.emoji ?? "📖"}</div>
                )}
                <div style={{ position: "absolute", top: 10, right: 10,
                  padding: "3px 10px", borderRadius: 20, fontSize: "0.72rem", fontWeight: 800,
                  background: "rgba(255,255,255,0.9)", color: statusInfo.color }}>
                  {s.status === "generating" && (
              ...
              <div style={{ padding: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 12 }}>
                  <button onClick={() => { setReadingStory(s); setView("read"); }}
                    disabled={s.status === "generating"}
                    style={{ padding: "9px 0", borderRadius: 8, border: "none",
                      background: s.status === "ready" || s.status === "published" ? "var(--primary)" : "var(--border)",
                      color: s.status === "ready" || s.status === "published" ? "#fff" : "var(--text-muted)",
                      cursor: s.status === "generating" ? "not-allowed" : "pointer",
                  ...
                  <button onClick={() => handleRegenerate(s.id)} disabled={s.status === "generating"}
                    style={{ padding: "8px 0", borderRadius: 8, border: "1.5px solid var(--border)",
                      background: "var(--surface)", cursor: s.status === "generating" ? "not-allowed" : "pointer",
                      fontWeight: 600, fontSize: "0.78rem", fontFamily: "var(--font-body)" }}>
                    🔄 Tạo lại
                  </button>
                  <button onClick={() => handleDelete(s.id, s.title)}
                    style={{ padding: "8px 0", borderRadius: 8, border: "1.5px solid #fee2e2",
                      background: "#fff5f5", cursor: "pointer",
                      fontWeight: 600, fontSize: "0.78rem", fontFamily: "var(--font-body)", color: "#dc2626" }}>
                    🗑️ Xóa
                  </button>
                </div>
              </div>
