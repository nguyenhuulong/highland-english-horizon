...
export default function CreatorEditor() {
  ...
  const addDialogue = (panelIdx: number) =>
    setPanels((prev) =>
      prev.map((p, i) =>
        i === panelIdx ? { ...p, dialogues: [...p.dialogues, { char: "", vi: "", en: "" }] } : p
      )
    );
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <FormGroup label="Dân tộc">
            <select style={inputStyle} value={ethnic} onChange={(e) => setEthnic(e.target.value)}>
              {["K'Ho", "Mạ", "M'Nông", "H'Mông", "Tày", "Nùng"].map((e) => (
      ...
      {panels.map((panel, pi) => (
        <div key={pi} style={{ background: "var(--surface)", borderRadius: 16, padding: 20, marginBottom: 16, border: "2px dashed var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            {pi > 0 && (
              <button onClick={() => removePanel(pi)} style={{ padding: "6px 14px", borderRadius: 8, background: "#FFEBEE", color: "#F44336", border: "none", cursor: "pointer", fontWeight: 700, fontFamily: "var(--font-body)" }}>
                ✕ Xóa
              </button>
            )}
          </div>
          ...
          <button onClick={() => addDialogue(pi)} style={{ padding: "7px 15px", borderRadius: 8, background: "var(--surface)", color: "var(--text)", border: "1.5px solid var(--border)", cursor: "pointer", fontWeight: 700, fontFamily: "var(--font-body)" }}>
            + Thêm lời thoại
          </button>
        </div>
      ))}
