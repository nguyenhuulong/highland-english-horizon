...
export function useProgress() {
  ...
  function save(next: AllProgress) {
    try {
  ...
  const saveMatchScore = useCallback(
    (storyId: string, score: number) => {
      setProgress(prev =>
        save({ ...prev, [storyId]: { ...prev[storyId], matchScore: score } }),
      );
  ...
  const saveFillScore = useCallback(
    (storyId: string, score: number, total: number) => {
      setProgress(prev =>
        save({
          ...prev,
          [storyId]: { ...prev[storyId], quizScore: score, quizTotal: total },
        }),
      );
      if (isAuthed)
        syncToServer({ lessonId: storyId, action: "fill_blank", score, total });
    },
    [isAuthed],
  );

  const saveSpeakGameScore = useCallback(
    (
      storyId: string,
      score: number,
      sentenceEn: string,
      ...
      setProgress(prev =>
        save({ ...prev, [storyId]: { ...prev[storyId], pronScore: score } }),
      );
      if (isAuthed)
        syncToServer({
          lessonId: storyId,
